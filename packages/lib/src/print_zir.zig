...
pub fn render(
    ...
    var raw_stream = std.io.bufferedWriter(fs_writer);
    const stream = raw_stream.writer();
...
const Writer = struct {
    ...
    pub fn nameExists(name: Zir.Inst.Declaration.Name, zir: Zir) bool {
        return @intFromEnum(name) < zir.string_bytes.len;
    }
    ...
    fn writeDeclaration(self: *Writer, stream: anytype, inst: Zir.Inst.Index) !void {
        ...
        if (inst_data.payload_index > self.code.extra.len) {
            try stream.writeAll("invalid) ");
            try self.writeSrcNode(stream, 0);
            // try stream.print("invalid declaration payload index: {any}\n", .{inst_data});
            return;
            // @panic(std.fmt.format("invalid declaration payload index: {any}\n", .{inst_data}));
        }
        ...
        switch (extra.data.name) {
            ...
            _ => {
                if (nameExists(extra.data.name, self.code)) if (extra.data.name.toString(self.code)) |name| {
    ...
    fn writeInstRef(self: *Writer, stream: anytype, ref: Zir.Inst.Ref) !void {
        if (ref == .none) {
            ...
            const val: ZcuIndex = @enumFromInt(@intFromEnum(ref));

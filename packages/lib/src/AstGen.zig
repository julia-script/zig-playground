...
const ResultInfo = struct {
    ...
    const Loc = union(enum) {
        ...
        fn resultTypeForCast(rl: Loc, gz: *GenZir, node: Ast.Node.Index, builtin_name: []const u8) !Zir.Inst.Ref {
            ...
            switch (rl) {
                .destructure => |destructure| astgen.failNodeNotes(node, "{s} must have a known result type", .{builtin_name}, &.{
                ...
                else => astgen.failNodeNotes(node, "{s} must have a known result type", .{builtin_name}, &.{
                    try astgen.errNoteNode(node, "use @as to provide explicit result type", .{}),
                }) catch {},
            }
            return RefUnknown;
...
fn lvalExpr(gz: *GenZir, scope: *Scope, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    switch (node_tags[node]) {
        ...
        .error_value,
        => {
            astgen.failNode(node, "invalid left-hand side to assignment", .{}) catch {};
            return RefUnknown;
        },
...
fn suspendExpr(
    ...
    if (gz.nosuspend_node != 0) {
        astgen.failNodeNotes(node, "suspend inside nosuspend block", .{}, &[_]u32{
            try astgen.errNoteNode(gz.nosuspend_node, "nosuspend block here", .{}),
        }) catch {};
        return RefUnknown;
    }
    if (gz.suspend_node != 0) {
        astgen.failNodeNotes(node, "cannot suspend inside suspend block", .{}, &[_]u32{
...
fn awaitExpr(
    ...
    if (gz.suspend_node != 0) {
        astgen.failNodeNotes(node, "cannot await inside suspend block", .{}, &[_]u32{
...
fn fnProtoExpr(
    ...
    if (fn_proto.name_token) |some| {
        astgen.failTok(some, "function type cannot have a name", .{}) catch {};
    }
            ...
            const is_comptime = if (param.comptime_noalias) |token| switch (token_tags[token]) {
                .keyword_noalias => is_comptime: {
                    noalias_bits |= @as(u32, 1) << (std.math.cast(u5, param_type_i) orelse
                        return astgen.failTok(token, "this compiler implementation only supports 'noalias' on the first 32 parameters", .{})); // TODO
    ...
    if (fn_proto.ast.align_expr != 0) {
        astgen.failNode(fn_proto.ast.align_expr, "function type cannot have an alignment", .{}) catch {};
    }

    if (fn_proto.ast.addrspace_expr != 0) {
        astgen.failNode(fn_proto.ast.addrspace_expr, "function type cannot have an addrspace", .{}) catch {};
    }

    if (fn_proto.ast.section_expr != 0) {
        astgen.failNode(fn_proto.ast.section_expr, "function type cannot have a linksection", .{}) catch {};
    }
    ...
    if (is_inferred_error) {
        astgen.failTok(maybe_bang, "function type cannot have an inferred error set", .{}) catch {};
    }
    ...
    switch (ri.rl) {
        ...
        .destructure => |destructure| {
            ...
            if (array_init.ast.elements.len != destructure.components.len) {
                astgen.failNodeNotes(node, "expected {} elements for destructure, found {}", .{
...
fn structInitExpr(
    ...
    if (struct_init.ast.type_expr == 0) {
        if (struct_init.ast.fields.len == 0) {
            ...
            switch (ri.rl) {
                ...
                .destructure => |destructure| {
                    astgen.failNodeNotes(node, "empty initializer cannot be destructured", .{}, &.{
                        try astgen.errNoteNode(destructure.src_node, "result destructured here", .{}),
                    }) catch {};
                },
            }
    ...
    switch (ri.rl) {
        ...
        .destructure => |destructure| {
            ...
            astgen.failNodeNotes(node, "struct value cannot be destructured", .{}, &.{
                try astgen.errNoteNode(destructure.src_node, "result destructured here", .{}),
            }) catch {};
            return RefUnknown;
        },
...
fn comptimeExprAst(
    ...
    const astgen = gz.astgen;
    if (gz.is_comptime) {
        astgen.failNode(node, "redundant comptime keyword in already comptime scope", .{}) catch {};
    }
    const tree = astgen.tree;
    const node_datas = tree.nodes.items(.data);
    const body_node = node_datas[node].lhs;
...
fn breakExpr(parent_gz: *GenZir, parent_scope: *Scope, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
        ...
        switch (scope.tag) {
            .gen_zir => {
                ...
                if (block_gz.cur_defer_node != 0) {
                    ...
                    astgen.failNodeNotes(node, "cannot break out of defer expression", .{}, &.{
    ...
    if (break_label != 0) {
        ...
        astgen.failTok(break_label, "label not found: '{s}'", .{label_name}) catch {};
    } else {
        astgen.failNode(node, "break expression outside loop", .{}) catch {};
    }
    return RefUnknown;
}
...
fn continueExpr(parent_gz: *GenZir, parent_scope: *Scope, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
        ...
        switch (scope.tag) {
            .gen_zir => {
                ...
                if (gen_zir.cur_defer_node != 0) {
                    astgen.failNodeNotes(node, "cannot continue out of defer expression", .{}, &.{
                        try astgen.errNoteNode(
    ...
    if (break_label != 0) {
        ...
        astgen.failNode(node, "continue expression outside loop", .{}) catch {};
    }
    return RefUnknown;
}
            ...
            .gen_zir => {
                ...
                if (gen_zir.label) |prev_label| {
                    if (try astgen.tokenIdentEql(label, prev_label.token)) {
                        ...
                        astgen.failTokNotes(label, "redefinition of label '{s}'", .{
                            label_name,
                        }, &[_]u32{
...
fn varDecl(
    ...
    if (mem.eql(u8, ident_name_raw, "_")) {
        astgen.failTok(name_token, "'_' used as an identifier without @\"_\" syntax", .{}) catch {};
    }
    ...
    if (var_decl.ast.init_node == 0) {
        astgen.failNode(node, "variables must be initialized", .{}) catch {};
    }

    if (var_decl.ast.addrspace_node != 0) {
        astgen.failTok(main_tokens[var_decl.ast.addrspace_node], "cannot set address space of local variable '{s}'", .{ident_name_raw}) catch {};
    }

    if (var_decl.ast.section_node != 0) {
        astgen.failTok(main_tokens[var_decl.ast.section_node], "cannot set section of local variable '{s}'", .{ident_name_raw}) catch {};
    }
    ...
    switch (token_tags[var_decl.ast.mut_token]) {
        .keyword_const => {
            if (var_decl.comptime_token) |comptime_token| {
                astgen.appendErrorTok(comptime_token, "'comptime const' is redundant; instead wrap the initialization expression with 'comptime'", .{}) catch {};
            }
        ...
        .keyword_var => {
            if (var_decl.comptime_token != null and gz.is_comptime)
                astgen.failTok(var_decl.comptime_token.?, "'comptime var' is redundant in comptime scope", .{}) catch {};
        ...
        switch (node_tags[variable_node]) {
            ...
            .global_var_decl, .local_var_decl, .simple_var_decl, .aligned_var_decl => {
                ...
                if (full_var_decl.ast.addrspace_node != 0) {
                    astgen.failTok(main_tokens[full_var_decl.ast.addrspace_node], "cannot set address space of local variable '{s}'", .{ident_name_raw}) catch {};
                }
                if (full_var_decl.ast.section_node != 0) {
                    astgen.failTok(main_tokens[full_var_decl.ast.section_node], "cannot set section of local variable '{s}'", .{ident_name_raw}) catch {};
                }
...
fn ptrType(
    ...
    if (ptr_info.size == .C and ptr_info.allowzero_token != null) {
        gz.astgen.failTok(ptr_info.allowzero_token.?, "C pointers always allow address zero", .{}) catch {};
    }
...
fn arrayType(gz: *GenZir, scope: *Scope, ri: ResultInfo, node: Ast.Node.Index) !Zir.Inst.Ref {
    ...
    {
        astgen.failNode(len_node, "unable to infer array size", .{}) catch {};
        return RefUnknown;
    }
...
fn fnDecl(
    ...
    const is_var_args = is_var_args: {
        ...
        while (it.next()) |param| : (param_type_i += 1) {
            ...
            } else if (!is_extern) blk: {
                if (param.anytype_ellipsis3) |tok| {
                    astgen.failTok(tok, "missing parameter name", .{}) catch {};
                } else {
                    ambiguous: {
                    ...
                    astgen.failNode(param.type_expr, "missing parameter name", .{}) catch {};
                }
                break :blk .empty;
            } else .empty;
    ...
    const lib_name = if (fn_proto.lib_name) |lib_name_token| blk: {
        ...
        if (mem.indexOfScalar(u8, lib_name_slice, 0) != null) {
            astgen.failTok(lib_name_token, "library name cannot contain null bytes", .{}) catch {};
        } else if (lib_name_str.len == 0) {
            astgen.failTok(lib_name_token, "library name cannot be empty", .{}) catch {};
        }
    ...
    const func_inst: Zir.Inst.Ref = if (body_node == 0) func: {
        if (!is_extern) {
            astgen.failTok(fn_proto.ast.fn_token, "non-extern function has no body", .{}) catch {};
        }
        if (is_inferred_error) {
            astgen.failTok(maybe_bang, "function prototype may not have inferred error set", .{}) catch {};
        }
...
fn globalVarDecl(
    ...
    const is_threadlocal = if (var_decl.threadlocal_token) |tok| blk: {
        if (!is_mutable) {
            astgen.failTok(tok, "threadlocal variable cannot be constant", .{}) catch {};
        }
        break :blk true;
    } else false;
    ...
    const var_inst: Zir.Inst.Ref = if (var_decl.ast.init_node != 0) vi: {
        ...
        astgen.failNode(node, "unable to infer variable type", .{}) catch {};
        break :vi RefUnknown;
    };
...
fn testDecl(
    ...
    const test_name: DeclarationName = switch (token_tags[test_name_token]) {
        ...
        .identifier => blk: {
            ...
            if (mem.eql(u8, ident_name_raw, "_")) astgen.failTok(test_name_token, "'_' used as an identifier without @\"_\" syntax", .{}) catch {};

            // if not @"" syntax, just use raw token slice
            if (ident_name_raw[0] != '@') {
                if (isPrimitive(ident_name_raw)) astgen.failTok(test_name_token, "cannot test a primitive", .{}) catch {};
            }
            ...
            while (true) switch (s.tag) {
                .local_val => {
                    ...
                    if (local_val.name == name_str_index) {
                        ...
                        astgen.failTokNotes(test_name_token, "cannot test a {s}", .{
                ...
                .namespace => {
                    ...
                    if (ns.decls.get(name_str_index)) |i| {
                        if (found_already) |f| {
                            astgen.failTokNotes(test_name_token, "ambiguous reference", .{}, &.{
            ...
            if (found_already == null) {
                ...
                astgen.failTok(test_name_token, "use of undeclared identifier '{s}'", .{ident_name}) catch {};
            }
...
fn structDeclInner(
    ...
    const backing_int_ref: Zir.Inst.Ref = blk: {
        if (backing_int_node != 0) {
            if (layout != .@"packed") {
                astgen.failNode(backing_int_node, "non-packed struct does not support backing integer type", .{}) catch {};
            }
    ...
    if (is_tuple) switch (layout) {
        .auto => {},
        .@"extern" => astgen.failNode(node, "extern tuples are not supported", .{}) catch {},
        .@"packed" => astgen.failNode(node, "packed tuples are not supported", .{}) catch {},
    };
    ...
    for (container_decl.ast.members) |member_node| {
        ...
        if (!is_tuple) {
            ...
            astgen.failTok(member.ast.main_token, "tuple field has a name", .{}) catch {};
        }
        ...
        if (member.ast.type_expr == 0) {
            astgen.failTok(member.ast.main_token, "struct field missing type", .{}) catch {};
        }
        ...
        if (is_comptime) {
            switch (layout) {
                .@"packed" => astgen.failTok(member.comptime_token.?, "packed struct fields cannot be marked comptime", .{}) catch {},
                .@"extern" => astgen.failTok(member.comptime_token.?, "extern struct fields cannot be marked comptime", .{}) catch {},
...
fn unionDeclInner(
    ...
    if (layout != .auto and (auto_enum_tok != null or arg_node != 0)) {
        if (arg_node != 0) {
            astgen.failNode(arg_node, "{s} union does not support enum tag type", .{@tagName(layout)}) catch {};
        } else {
            astgen.failTok(auto_enum_tok.?, "{s} union does not support enum tag type", .{@tagName(layout)}) catch {};
        }
    }
    ...
    for (members) |member_node| {
        ...
        if (member.ast.tuple_like) {
            astgen.failTok(member.ast.main_token, "union field missing name", .{}) catch {};
        }
        if (member.comptime_token) |comptime_token| {
            astgen.failTok(comptime_token, "union fields cannot be marked comptime", .{}) catch {};
        }
        ...
        if (have_type) {
            ...
            astgen.failNode(member_node, "union field missing type", .{}) catch {};
        }
        if (have_align) {
    ...
    switch (token_tags[container_decl.ast.main_token]) {
        ...
        .keyword_enum => {
            if (container_decl.layout_token) |t| {
                astgen.failTok(t, "enums do not support 'packed' or 'extern'; instead provide an explicit integer tag type", .{}) catch {};
            }
            ...
            const counts = blk: {
                ...
                for (container_decl.ast.members) |member_node| {
                    ...
                    member.convertToNonTupleLike(astgen.tree.nodes);
                    if (member.ast.tuple_like) {
                        astgen.failTok(member.ast.main_token, "enum field missing name", .{}) catch {};
                    }
                    if (member.comptime_token) |comptime_token| {
                        astgen.failTok(comptime_token, "enum fields cannot be marked comptime", .{}) catch {};
                    }
                    if (member.ast.type_expr != 0) {
                        astgen.failNodeNotes(
                    ...
                    if (member.ast.align_expr != 0) {
                        astgen.failNode(member.ast.align_expr, "enum fields cannot be aligned", .{}) catch {};
                    }
                    ...
                    if (mem.eql(u8, tree.tokenSlice(name_token), "_")) {
                        ...
                        if (member.ast.value_expr != 0) {
                            astgen.failNode(member.ast.value_expr, "'_' is used to mark an enum as non-exhaustive and cannot be assigned a value", .{}) catch {};
                        }
                        continue;
                    ...
                    if (member.ast.value_expr != 0) {
                        if (container_decl.ast.arg == 0) {
                            astgen.failNode(member.ast.value_expr, "value assigned to enum tag with inferred tag type", .{}) catch {};
                        }
                        values += 1;
                    }
                }
                if (nonfinal_nonexhaustive) {
                    astgen.failNode(nonexhaustive_node, "'_' field of non-exhaustive enum must be last", .{}) catch {};
                }
                break :blk .{
        ...
        .keyword_opaque => {
            ...
            for (container_decl.ast.members) |member_node| {
                ...
                if (res == .field) {
                    astgen.failNode(member_node, "opaque types cannot have fields", .{}) catch {};
                }
            }
...
fn tryExpr(
    ...
    const fn_block = astgen.fn_block orelse blk: {
        astgen.failNode(node, "'try' outside function scope", .{}) catch {};
        break :blk null;
    };

    if (parent_gz.any_defer_node != 0) {
        astgen.failNodeNotes(node, "'try' not allowed inside defer expression", .{}, &.{
            try astgen.errNoteNode(
    ...
    if (fn_block) |block| try genDefers(&else_scope, &block.base, scope, .{ .both = err_code });
...
fn orelseCatchExpr(
    ...
    const else_sub_scope = blk: {
        ...
        if (mem.eql(u8, err_str, "_")) {
            astgen.failTok(payload, "discard of error capture; omit it instead", .{}) catch {};
        }
...
fn simpleBinOp(
    ...
    if (op_inst_tag == .cmp_neq or op_inst_tag == .cmp_eq) {
        ...
        if (node_tags[node_datas[node].lhs] == .string_literal or
            ...
            astgen.failNode(node, "cannot compare strings with {s}", .{str}) catch {};
    }
...
fn whileExpr(
    ...
    if (parent_gz.is_comptime and is_inline) {
        astgen.failTok(while_full.inline_token.?, "redundant inline keyword in comptime scope", .{}) catch {};
    }
...
fn forExpr(
    ...
    if (parent_gz.is_comptime and is_inline) {
        astgen.failTok(for_full.inline_token.?, "redundant inline keyword in comptime scope", .{}) catch {};
    }
    const tree = astgen.tree;
    const token_tags = tree.tokens.items(.tag);
    const node_tags = tree.nodes.items(.tag);
...

    {
        ...
        for (for_full.ast.inputs, indexables, lens) |input, *indexable_ref, *len_ref| {
            ...
            if (is_discard and capture_is_ref) {
                astgen.failTok(capture_token, "pointer modifier invalid on discard", .{}) catch {};
            }
            ...
            if (node_tags[input] == .for_range) {
                if (capture_is_ref) {
                    astgen.failTok(ident_tok, "cannot capture reference to range", .{}) catch {};
                }
                ...
                if (end_val == .none and is_discard) {
                    astgen.failTok(ident_tok, "discard of unbounded counter", .{}) catch {};
                }
    ...
    if (!any_len_checks) {
        astgen.failNode(node, "unbounded for loop", .{}) catch {};
    }
...
fn switchExprErrUnion(
    ...
    for (case_nodes) |case_node| {
        ...
        for (case.ast.values) |val| {
            if (node_tags[val] == .string_literal)
                astgen.failNode(val, "cannot switch on strings", .{}) catch {};
        }
    ...
    const err_name = blk: {
        ...
        if (mem.eql(u8, err_str, "_")) {
            astgen.failTok(error_payload, "discard of error capture; omit it instead", .{}) catch {};
        }
    ...
    for (case_nodes) |case_node| {
        ...
        const sub_scope = blk: {
            ...
            if (token_tags[capture_token] != .identifier) {
                astgen.failTok(capture_token + 1, "error set cannot be captured by reference", .{}) catch {};
            }

            const capture_slice = tree.tokenSlice(capture_token);
            if (mem.eql(u8, capture_slice, "_")) {
                astgen.failTok(capture_token, "discard of error capture; omit it instead", .{}) catch {};
            }
...
fn switchExpr(
    ...
    for (case_nodes) |case_node| {
            ...
            mem.eql(u8, tree.tokenSlice(main_tokens[case.ast.values[0]]), "_"))
        {
            ...
            if (case.inline_token != null) {
                astgen.failTok(case_src, "cannot inline '_' prong", .{}) catch {};
            }
            special_node = case_node;
            special_prong = .under;
    ...
    for (case_nodes) |case_node| {
        ...
        const sub_scope = blk: {
            ...
            if (mem.eql(u8, ident_slice, "_")) {
                if (is_ptr) {
                    astgen.failTok(payload_token, "pointer modifier invalid on discard", .{}) catch {};
                }
            ...
            if (mem.eql(u8, tag_slice, "_")) {
                astgen.failTok(tag_token, "discard of tag capture; omit it instead", .{}) catch {};
            } else if (case.inline_token == null) {
                astgen.failTok(tag_token, "tag capture on non-inline prong", .{}) catch {};
            }
...
fn ret(gz: *GenZir, scope: *Scope, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    if (astgen.fn_block == null) {
        astgen.failNode(node, "'return' outside function scope", .{}) catch {};
    }

    if (gz.any_defer_node != 0) {
        astgen.failNodeNotes(node, "cannot return from defer expression", .{}, &.{
            try astgen.errNoteNode(
                gz.any_defer_node,
                "defer expression here",
                .{},
...
fn identifier(
    ...
    const ident_name_raw = tree.tokenSlice(ident_token);
    if (mem.eql(u8, ident_name_raw, "_")) {
        astgen.failNode(ident, "'_' used as an identifier without @\"_\" syntax", .{}) catch {};
    }
        ...
        if (ident_name_raw.len >= 2) integer: {
            ...
            if (first_c == 'i' or first_c == 'u') {
                ...
                const bit_count = parseBitCount(ident_name_raw[1..]) catch |err| switch (err) {
                    error.Overflow => astgen.failNode(
                        ident,
...
fn localVarRef(
    ...
    while (true) switch (s.tag) {
        ...
        .local_ptr => {
            ...
            if (local_ptr.name == name_str_index) {
                ...
                if (num_namespaces_out != 0 and !local_ptr.maybe_comptime and !gz.is_typeof) {
                    ...
                    astgen.failNodeNotes(ident, "mutable '{s}' not accessible from here", .{ident_name}, &.{
        ...
        .namespace => {
            ...
            if (ns.decls.get(name_str_index)) |i| {
                if (found_already) |f| {
                    astgen.failNodeNotes(ident, "ambiguous reference", .{}, &.{
                        try astgen.errNoteNode(f, "declared here", .{}),
                        try astgen.errNoteNode(i, "also declared here", .{}),
                    }) catch {};
                }
    ...
    if (found_already == null) {
        const ident_name = try astgen.identifierTokenString(ident_token);
        astgen.failNode(ident, "use of undeclared identifier '{s}'", .{ident_name}) catch {};
    }
...
fn tunnelThroughClosure(
    ...
    ) orelse astgen.failNodeNotes(root_ns.node, "this compiler implementation only supports up to 65536 captures per namespace", .{}, &.{
        switch (decl_src) {
    ...
    for (intermediate_tunnels) |tunnel_ns| {
        ...
        ) orelse astgen.failNodeNotes(tunnel_ns.node, "this compiler implementation only supports up to 65536 captures per namespace", .{}, &.{
            switch (decl_src) {
                .token => |t| try astgen.errNoteTok(t, "captured value here", .{}),
                .node => |n| try astgen.errNoteNode(n, "captured value here", .{}),
            },
...
fn numberLiteral(gz: *GenZir, ri: ResultInfo, node: Ast.Node.Index, source_node: Ast.Node.Index, sign: Sign) InnerError!Zir.Inst.Ref {
    ...
    const result: Zir.Inst.Ref = switch (std.zig.parseNumberLiteral(bytes)) {
        .int => |num| switch (num) {
            0 => blk: {
                if (sign != .positive) astgen.failTokNotes(num_token, "integer literal '-0' is ambiguous", .{}, &.{
        ...
        .failure => |err| astgen.failWithNumberError(err, num_token, bytes) catch return RefUnknown,
    };
...
fn asmExpr(
    ...
    if (is_container_asm) {
        if (full.volatile_token) |t|
            astgen.failTok(t, "volatile is meaningless on global assembly", .{}) catch {};
        if (full.outputs.len != 0 or full.inputs.len != 0 or full.first_clobber != null)
            astgen.failNode(node, "global assembly cannot have inputs, outputs, or clobbers", .{}) catch {};
    } else {
        if (full.outputs.len == 0 and full.volatile_token == null) {
            astgen.failNode(node, "assembly expression with no output must be marked volatile", .{}) catch {};
        }
    }
    if (full.outputs.len > 32) {
        astgen.failNode(full.outputs[32], "too many asm outputs", .{}) catch {};
    }
    ...
    for (full.outputs, 0..) |output_node, i| {
        ...
        if (has_arrow) {
            if (output_type_bits != 0) {
                astgen.failNode(output_node, "inline assembly allows up to one output value", .{}) catch {};
            }
    ...
    if (full.inputs.len > 32) {
        astgen.failNode(full.inputs[32], "too many asm inputs", .{}) catch {};
    }
    ...
    if (full.first_clobber) |first_clobber| clobbers: {
        ...
        while (true) : (tok_i += 1) {
            if (clobber_i >= clobbers_buffer.len) {
                astgen.failTok(tok_i, "too many asm clobbers", .{}) catch {};
            }
            ...
            switch (info.tag) {
                ...
                => |tag| {
                    if (@field(flags, @tagName(tag))) {
                        astgen.failNode(node, "redundant {s}", .{builtin_name}) catch {};
                    }
...
fn typeOf(
    ...
    const astgen = gz.astgen;
    if (args.len < 1) {
        astgen.failNode(node, "expected at least 1 argument, found 0", .{}) catch {};
    }
    const gpa = astgen.gpa;
    if (args.len == 1) {
...
fn minMax(
    ...
    const astgen = gz.astgen;
    if (args.len < 2) {
        astgen.failNode(node, "expected at least 2 arguments, found 0", .{}) catch {};
    }
    if (args.len == 2) {
...
fn builtinCall(
    ...
    const info = BuiltinFn.list.get(builtin_name) orelse {
        astgen.failNode(node, "invalid builtin function: '{s}'", .{
            builtin_name,
        }) catch return RefUnknown;
    };
    if (info.param_count) |expected| {
        if (expected != params.len) {
            ...
            astgen.failNode(node, "expected {d} argument{s}, found {d}", .{
    ...
    if (astgen.fn_block == null and info.illegal_outside_function)
        astgen.failNode(node, "'{s}' outside function scope", .{builtin_name}) catch {};
    ...
    switch (info.tag) {
        .import => {
            ...
            if (node_tags[operand_node] != .string_literal) {
                ...
                astgen.failNode(operand_node, "@import operand must be a string literal", .{}) catch {};
            }
            ...
            if (mem.indexOfScalar(u8, str_slice, 0) != null) {
                astgen.failTok(str_lit_token, "import path cannot contain null bytes", .{}) catch {};
            } else if (str.len == 0) {
                astgen.failTok(str_lit_token, "import path cannot be empty", .{}) catch {};
            }
            ...
            switch (node_tags[params[0]]) {
                .identifier => {
                    ...
                    if (isPrimitive(tree.tokenSlice(ident_token))) {
                        astgen.failTok(ident_token, "unable to export primitive value", .{}) catch {};
                    }
                    ...
                    while (true) switch (s.tag) {
                        ...
                        .local_ptr => {
                            ...
                            if (local_ptr.name == decl_name) {
                                if (!local_ptr.maybe_comptime)
                                    astgen.failNode(params[0], "unable to export runtime-known value", .{}) catch {};
                                local_ptr.used = ident_token;
                        ...
                        .namespace => {
                            ...
                            if (ns.decls.get(decl_name)) |i| {
                                if (found_already) |f| {
                                    astgen.failNodeNotes(node, "ambiguous reference", .{}, &.{
                                        try astgen.errNoteNode(f, "declared here", .{}),
                                        try astgen.errNoteNode(i, "also declared here", .{}),
                                    }) catch {};
                                }
                    ...
                    if (found_already == null) {
                        const ident_name = try astgen.identifierTokenString(ident_token);
                        astgen.failNode(params[0], "use of undeclared identifier '{s}'", .{ident_name}) catch {};
                    }
                },
                .field_access => {
                ...
                else => astgen.failNode(params[0], "symbol to export must identify a declaration", .{}) catch {},
            }
        ...
        .in_comptime => if (gz.is_comptime) {
            astgen.failNode(node, "redundant '@inComptime' in comptime scope", .{}) catch return RefUnknown;
        } else {
        ...
        .c_define => {
            if (!gz.c_import) gz.astgen.failNode(node, "C define valid only inside C import block", .{}) catch {};
        ...
        .c_va_start => {
            if (!astgen.fn_var_args) {
                astgen.failNode(node, "'@cVaStart' in a non-variadic function", .{}) catch {};
            }
...
fn simpleCBuiltin(
    ...
    if (!gz.c_import) gz.astgen.failNode(node, "{s} valid only inside C import block", .{name}) catch {};
...
fn cImport(
    ...
    if (gz.c_import) gz.astgen.failNode(node, "cannot nest @cImport", .{}) catch {};
...
fn identifierTokenString(astgen: *AstGen, token: Ast.TokenIndex) InnerError![]const u8 {
    ...
    if (mem.indexOfScalar(u8, buf.items, 0) != null) {
        astgen.failTok(token, "identifier cannot contain null bytes", .{}) catch {};
    } else if (buf.items.len == 0) {
        astgen.failTok(token, "identifier cannot be empty", .{}) catch {};
    }
...
fn testNameString(astgen: *AstGen, str_lit_token: Ast.TokenIndex) !Zir.NullTerminatedString {
    ...
    if (mem.indexOfScalar(u8, slice, 0) != null) {
        astgen.failTok(str_lit_token, "test name cannot contain null bytes", .{}) catch {};
    } else if (slice.len == 0) {
        astgen.failTok(str_lit_token, "empty test name must be omitted", .{}) catch {};
    }
    try string_bytes.append(gpa, 0);
    return @enumFromInt(str_index);
}
...
fn detectLocalShadowing(
    ...
    if (token_bytes[0] != '@' and isPrimitive(token_bytes)) {
        astgen.failTokNotes(name_token, "name shadows primitive '{s}'", .{
            token_bytes,
        }, &[_]u32{
    ...
    while (true) switch (s.tag) {
        .local_val => {
            ...
            if (local_val.name == ident_name) {
                ...
                defer gpa.free(name);
                if (outer_scope) {
                    astgen.failTokNotes(name_token, "{s} '{s}' shadows {s} from outer scope", .{
                ...
                astgen.failTokNotes(name_token, "redeclaration of {s} '{s}'", .{
        ...
        .namespace => {
            ...
            const name = try gpa.dupe(u8, name_slice);
            defer gpa.free(name);
            astgen.failTokNotes(name_token, "{s} shadows declaration of '{s}'", .{
                @tagName(id_cat), name,
            }, &[_]u32{
    ...
    for (members) |member_node| {
        ...
        while (true) switch (s.tag) {
            .local_val => {
                const local_val = s.cast(Scope.LocalVal).?;
                if (local_val.name == name_str_index) {
                    astgen.failTokNotes(name_token, "declaration '{s}' shadows {s} from outer scope", .{
...
pub fn generateLsp(gpa: Allocator, tree: Ast) Allocator.Error!Zir {
    var arena = std.heap.ArenaAllocator.init(gpa);
    defer arena.deinit();
...
const RefUnknown: Zir.Inst.Ref = @enumFromInt(std.math.maxInt(u32) - 1);
